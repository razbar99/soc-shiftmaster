...
def init_db():
    ...
    conn.execute('CREATE TABLE IF NOT EXISTS shifts (date TEXT, type TEXT, staff TEXT, hours TEXT, is_draft INTEGER DEFAULT 1, PRIMARY KEY (date, type))')
...
def save_shift(shift: dict):
    conn = sqlite3.connect(DB_NAME)
    conn.execute("INSERT OR REPLACE INTO shifts (date, type, staff, hours, is_draft) VALUES (?, ?, ?, ?, 1)", (shift['date'], shift['shift_type'], shift['staff'], '00:00'))
    conn.commit(); conn.close()
...
@app.delete("/api/shifts/{date}/{type}")
def del_shift(date, type):
    conn = sqlite3.connect(DB_NAME)
    conn.execute("DELETE FROM shifts WHERE date=? AND type=?", (date, type))
    conn.commit(); conn.close()
...
def add_req(req: dict):
    conn = sqlite3.connect(DB_NAME)
    conn.execute("INSERT INTO requests (name, email, date, req_type, reason) VALUES (?, ?, ?, ?, ?)", (req['name'], req['email'], req['date'], req['req_type'], req['reason']))
    conn.commit(); conn.close()
    return {"status": "success"}
...
def get_h():
    ...
    data = [dict(r) for r in conn.execute("SELECT * FROM handovers ORDER BY id DESC LIMIT 10").fetchall()]
    conn.close(); return data

@app.post("/api/handovers")
def add_h(data: dict):
    conn = sqlite3.connect(DB_NAME)
    conn.execute("INSERT INTO handovers (date, author, content, timestamp) VALUES (?, ?, ?, ?)", (datetime.now().strftime("%Y-%m-%d"), data['author'], data['content'], datetime.now().strftime("%H:%M")))
    conn.commit(); conn.close()
    return {"status": "success"}
